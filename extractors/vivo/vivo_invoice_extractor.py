...
def extrair_vivo(pdf_path: str) -> dict:
    ...
    for linha in linhas:
        if re.search(r"Telefônica|Vivo", linha):
    ...
    for linha in linhas:
        if "Data de Emissão:" in linha:
            try:
                dados["data_emissao"] = datetime.strptime(linha.split("Data de Emissão:")[1].strip(), "%d/%m/%Y").date()
            except:
                pass
    ...
    match_serie = re.search(r"Série:\s*(\S+)", texto)
    if match_serie:
